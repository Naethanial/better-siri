...
- **Global Hotkey**: Press `Cmd + .` from anywhere to trigger the assistant
...
```bash
git clone https://github.com/Naethanial/better-siri
cd BetterSiri
```
...
2. **Open Settings** - Click the menubar icon -> Settings (or `Cmd + ,`)
3. **Configure**:
   - Enter your **OpenRouter API key**
   - Choose a **model** (default: `google/gemini-3-flash-preview`)
   - Optionally change the **hotkey**
4. **Grant Screen Recording Permission** - Press `Cmd + .` and follow the prompt

## Usage

| Action | Shortcut |
|--------|----------|
| Open/Close Panel | `Cmd + .` (configurable) |
| Send Message | `Enter` |
...
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  ...
  --user-data-dir="$HOME/Library/Application Support/BetterSiri/Chrome"
```
...
├── BetterSiri/              # Source code (Swift Package)
│   ├── Package.swift
│   └── Sources/
...
1. Ensure Xcode Command Line Tools are installed:
   ...
   ```
2. Check Swift version (requires Swift 6.0+):
   ```bash
   swift --version
   ```
3. Clean and rebuild:
